...
async function main(): Promise<void> {
  ...
  const summary: MigrationRunSummary = {
    ...
    rowsRead: 0,
    rowsWritten: 0,
    notes: ['Skeleton migration runner created. Implement source and target adapters before cutover.']
  };

  try {
    const outputPath = path.resolve(outputDir, `migration-run-${Date.now()}.json`);
