...
test.describe("List create/delete flow", () => {
  test("creates two lists and deletes one with selection fallback", async ({
    ...
    try {
      await expect(
        page.getByRole("button", { name: /Select list / }),
      ).toBeVisible({ timeout: 15000 });
      await expect(
        page.getByRole("button", { name: `Select list ${firstListName}` }),
    ...
    try {
      await expect(
        page.getByRole("button", { name: /Select list / }).first(),
      ).toBeVisible({ timeout: 15000 });
      await expect(
        page.getByRole("button", { name: `Select list ${secondListName}` }),
      ).toBeVisible({ timeout: 5000 });
    } catch {
