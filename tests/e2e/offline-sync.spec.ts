...
test.describe('Offline sync replay', () => {
  test('queues a mutation while offline and flushes when back online', async ({ page, context }) => {
    ...
    await expect(page.getByTestId('item-hydrated')).toHaveText('ready', { timeout: 15000 });
    ...
    await expect(page.getByTestId('sync-status')).toContainText('online', { timeout: 10000 });
    await expect(page.getByTestId('sync-status')).toContainText('synced', { timeout: 10000 });
  });
});
