...
test("cross-device contexts converge on latest item text", async ({
  browser,
}) => {
  ...
  await expect(pageA.getByTestId("hydrated")).toHaveText("ready", {
    timeout: 15000,
  });
  await expect(pageB.getByTestId("hydrated")).toHaveText("ready", {
    timeout: 15000,
  });
