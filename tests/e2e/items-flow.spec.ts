...
test.describe('Item lifecycle flow', () => {
  test('creates, edits, completes, and deletes an item', async ({ page }) => {
    ...
    await expect(page.locator('.item-text', { hasText: testItemName })).toBeVisible({ timeout: 10000 });

    // Wait a bit more for the UI to stabilize
    await page.waitForTimeout(1000);

    // Verify the item is visible
    await expect(page.locator('.item-text', { hasText: testItemName })).toBeVisible();
    ...
    await expect(page.locator('.item-text.completed', { hasText: testItemName })).toBeVisible({ timeout: 5000 });

    await page.getByRole('button', { name: `Delete item ${testItemName}` }).click();
    await expect(page.locator('.item-text', { hasText: testItemName })).toHaveCount(0, slowExpect);
  });
  ...
  test('renders incomplete-first alphabetical ordering and supports larger toggle target', async ({ page }) => {
    ...
    await expect(page.locator('.item-text.completed', { hasText: 'zebra' })).toBeVisible({ timeout: 10000 });
  });
  ...
  test('shows duplicate suggestions and supports reactivate-existing vs create-anyway', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Use existing item Buy milk' })).toBeVisible({ timeout: 20000 });

    await page.getByRole('button', { name: 'Use existing item Buy milk' }).click();
    ...
    await page.getByRole('button', { name: 'Add item' }).click();
    await expect(page.getByRole('button', { name: 'Create new anyway' })).toBeVisible({ timeout: 20000 });
